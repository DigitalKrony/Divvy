...
@router.post(
  '/', response_model=schemas.StandardResponse[schemas.UserResponse], status_code=201
)
...
@router.get(
  '/{user_uuid}',
  response_model=schemas.StandardResponse[schemas.UserResponse],
  status_code=200,
)
def get_user(user_uuid: str, db: Session = Depends(get_db)):
  ...
  return schemas.success_response(data=user, code=200)
...
def get_user_groups(user_uuid: str, db: Session = Depends(get_db)):
  ...
  if not user:
    raise HTTPException(status_code=404, detail='User not found')
  return schemas.success_response(data=user.groups, code=200)
...
def get_user_events(user_uuid: str, db: Session = Depends(get_db)):
  ...
  if not user:
    raise HTTPException(status_code=404, detail='User not found')
  return schemas.success_response(data=user.events, code=200)
